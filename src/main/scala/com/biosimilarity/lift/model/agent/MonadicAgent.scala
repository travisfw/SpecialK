...
{
  self : MonadicWireToTrgtConversion with MonadicGenerators with Reporting =>
	  ...
	  outerK : ( Unit => Unit ) =>
		...
		msg match {
		  case l@Left(
		    ...
		    if ( validate( jreq ) ) {
		      report( "calling handler on " + jreq, Severity.Trace )
		      k( l )
		    }
		  }
		  case r@Right(
		    ...
		    if ( validate( jrsp ) ) {
		      report( "calling handler on " + jrsp, Severity.Trace )
		      k( r )
		    }
		  }
		}
